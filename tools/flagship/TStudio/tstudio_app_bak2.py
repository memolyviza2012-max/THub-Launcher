...
from tstudio_ui_shared import SettingsDialog, PromptSettingsDialog, GlossaryWidget, ApiWorker
...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QMenu, QSpinBox, QProgressBar, QDockWidget, QMenuBar
)
...
class MergeTranslatedDialog(QDialog):
    def __init__(self, parent=None):
        ...
        btn_orig.setToolTip(_("tooltip_orig"))
        ...
        btn_trans.setToolTip(_("tooltip_trans"))
        ...
        btn_save.setToolTip(_("tooltip_save"))
        ...
        self.btn_execute.setToolTip(_("tooltip_execute"))
...
class TranslationOptionsDialog(QDialog):
    def __init__(self, parent, options):
        super().__init__(parent)
        self.setWindowTitle(_("select_style_title"))
        self.setMinimumSize(500, 280)
        self.selected_text = ""
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(_("select_style_label")))
        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        content = QWidget()
        cl = QVBoxLayout(content)
        self.bg = QButtonGroup(self)
        labels = [_("style_literal"), _("style_casual"), _("style_formal")]
        ...
        btn_ok.setToolTip(_("tooltip_ok"))
...
class TranslationStudio(QMainWindow):
    def __init__(self):
        ...
        self.workspace_dock = QDockWidget("📝 Main Workspace", self)
        self.workspace_dock.setObjectName("workspace_dock")
        self.workspace_dock.setAllowedAreas(Qt.DockWidgetArea.AllDockWidgetAreas)

        central = QWidget()
        self.workspace_dock.setWidget(central)
        ...
        self.cbo_profile.setMinimumWidth(100)
        ...
        btn_glos = QPushButton(_("glossary_dock_title"))
        btn_glos.setToolTip(_("tooltip_glos"))
        btn_glos.clicked.connect(self.open_glossary)
        row1.addWidget(btn_glos)
        
        top_layout.addLayout(row1)
        
        # Row 2: File Operations & Search (Simplified)
        row2 = QHBoxLayout()
        ...
        grp1 = QGroupBox(_("dock_source_title"))
        l1 = QVBoxLayout(grp1)
        ...
        l3 = QVBoxLayout(grp3)
        btn_row = QHBoxLayout()
        btn_prompts = QPushButton(_("btn_ai_prompts"))
        btn_prompts.setToolTip(_("tooltip_prompts"))
        ...
        self.btn_trans_opt.setToolTip(_("tooltip_trans_opt"))
        ...
        self.btn_trans_single = QPushButton(_("btn_retranslate_ai"))
        self.btn_trans_single.setToolTip(_("tooltip_trans_single"))
        ...
        self.btn_trans_special.setToolTip(_("tooltip_trans_special"))
        self.btn_trans_special.setStyleSheet("background:#a6e3a1; color:#1e1e2e; font-size:12px; padding:4px 8px;")
        menu = QMenu(self)
        
        action_idiom = menu.addAction(_("menu_idiom"))
        action_poem = menu.addAction(_("menu_poem"))
        action_quote = menu.addAction(_("menu_quote"))
    ...
    def setup_menu_bar(self):
        ...
        edit_menu.addAction(act_global)
        ...
        act_glos = QAction("📖 Toggle Glossary", self)
        act_glos.setToolTip(_("tooltip_act_glos"))
        act_glos.triggered.connect(self.open_glossary)
        view_menu.addAction(act_glos)
        
        view_menu.addSeparator()
        act_reset = QAction("🔄 Reset Layout", self)
        act_reset.setToolTip(_("tooltip_act_reset"))
    ...
    def set_layout_mode(self, mode):
        ...
        if mode == 'floating':
        ...
        elif mode == 'compact' or mode == 'standard':
            self.glossary_dock.setFloating(False)
            self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, self.glossary_dock)
            if hasattr(self, 'video_dock'):
                self.video_dock.setFloating(False)
            ...
            # Glossary under the workspace tabbed, or just bottom
            self.glossary_dock.setFloating(False)
            self.addDockWidget(Qt.DockWidgetArea.BottomDockWidgetArea, self.glossary_dock)
    ...
    def open_glossary(self):
        # If it exists and has no unsaved changes, we can refresh it
    ...
    def import_profile(self):
        file_path, _ = QFileDialog.getOpenFileName(self, _("import_profile_title"), "", "JSON Files (*.json)")
        if file_path:
            try:
                ...
                from PyQt6.QtWidgets import QInputDialog
                name, ok = QInputDialog.getText(self, _("import_profile_title"), _("import_profile_prompt"))
    ...
    def open_file_dialog(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            BASE_DIR,
            "Supported Files (*.csv *.lor *.locres *.tex *.bundle *.txt *.srt *.vtt *.*);;CSV Files (*.csv);;Subtitle Files (*.srt *.vtt);;Locres Files (*.locres *.lor);;Texture Files (*.tex);;Unity Bundles (*.bundle *.txt *.*);;All Files (*.*)"
        )
        if file_path:
    ...
    def export_origin_format(self):
        ...
        file_path, filter_str = QFileDialog.getSaveFileName(
            ...
            BASE_DIR,
            "SubRip Subtitle (*.srt);;Web Video Text Tracks (*.vtt);;All Files (*.*)"
        )
    ...
    def deploy_to_game(self):
        ...
        reply = QMessageBox.question(self, _("btn_deploy"), f"Are you sure you want to deploy to:\n{retail_path}?",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            try:
                ...
                QMessageBox.information(self, _("success_title"), f"Deployed {len(game_dict):,} strings to game successfully!")
            except Exception as e:
                QMessageBox.critical(self, _("error_title"), f"Failed to deploy:\n{e}")

    def closeEvent(self, event):
        if hasattr(self, 'model') and getattr(self.model, 'is_dirty', False):
            reply = QMessageBox.question(self, _("unsaved_changes_title"), _("unsaved_before_exit"), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No | QMessageBox.StandardButton.Cancel)
            if reply == QMessageBox.StandardButton.Yes:
                self.save_csv()
                event.accept()
