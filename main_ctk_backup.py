...
- แหล่งอ้างอิงข้อมูลม็อด: [อ้างอิงจากโฟลเดอร์ Modding-Knowledge]
...
══════════════════════════════════════════════════════
  ...
  ฐานความรู้ม็อด                   : Modding-Knowledge/ ภายใน Workspace
...
class SteamSearchPreviewDialog(ctk.CTkToplevel):
    def __init__(self, parent, proj, initial_search=""):
        super().__init__(parent)
        self.parent_app = parent
        self.proj = proj
        self.title(_("ค้นหาและพรีวิวรูปภาพจาก Steam"))
        ...
        self.ent_search = ctk.CTkEntry(self.top_frame, placeholder_text=_("ระบุชื่อเกม หรือ App ID..."), width=400)
        self.ent_search.pack(side="left", padx=(0, 10))
        self.ent_search.insert(0, initial_search)
        self.ent_search.bind("<Return>", lambda e: self.do_search())
        
        self.btn_search = ctk.CTkButton(self.top_frame, text=_("ค้นหา 🔍"), width=100, command=self.do_search)
        self.btn_search.pack(side="left")
        
        # Loading Label
        self.lbl_status = ctk.CTkLabel(self, text=_("กำลังรอการค้นหา..."), text_color="gray")
    ...
    def do_search(self):
        ...
        self.lbl_status.configure(text=f"กำลังค้นหา '{query}'...")
    ...
    def _search_thread(self, query):
        ...
        try:
            ...
            if not items:
                self.after(0, lambda: self.lbl_status.configure(text=f"ไม่พบผลลัพธ์สำหรับ '{query}'"))
                return
                
            self.after(0, lambda: self.lbl_status.configure(text=f"พบ {len(items)} ผลลัพธ์"))
            
            # Fetch images and build UI
            for item in items:
                appid = item.get("id")
                name = item.get("name")
                tiny_url = item.get("tiny_image")
                
                # Fetch tiny image
                img_data = None
                try:
                    if tiny_url:
                        ireq = urllib.request.Request(tiny_url, headers={'User-Agent': 'Mozilla/5.0'})
                        with urllib.request.urlopen(ireq) as iresp:
                            img_data = iresp.read()
                except:
                    pass
                
                # Build item UI in main thread
                self.after(0, self._add_result_item, appid, name, img_data)
                
        except Exception as e:
            self.after(0, lambda: self.lbl_status.configure(text=f"ข้อผิดพลาด: {e}"))
            
    def _add_result_item(self, appid, name, img_data):
        import io
        from PIL import Image
        import customtkinter as ctk
        ...
        if img_data:
            try:
                ...
                ctk_img = ctk.CTkImage(light_image=pil_img, dark_image=pil_img, size=(115, 43)) # Approximate capsule size
                lbl_img = ctk.CTkLabel(item_frame, image=ctk_img, text="")
                lbl_img.pack(side="left", padx=10, pady=10)
            except:
                ctk.CTkLabel(item_frame, text="[No Image]").pack(side="left", padx=10)
        else:
            ctk.CTkLabel(item_frame, text="[No Image]").pack(side="left", padx=10)
            
        ctk.CTkLabel(item_frame, text=f"{name}\nAppID: {appid}", justify="left", font=ctk.CTkFont(weight="bold")).pack(side="left", padx=10)
        
        btn_select = ctk.CTkButton(item_frame, text=_("เลือกภาพนี้ 🖼️"), fg_color="#a6e3a1", text_color="#11111b", hover_color="#94e2d5",
                                 command=lambda a=appid: self._on_select(a))
        btn_select.pack(side="right", padx=10)
        
    def _on_select(self, appid):
        self.lbl_status.configure(text=_("กำลังดาวน์โหลดภาพขนาดเต็ม..."))
        import threading
        threading.Thread(target=self._download_header, args=(appid,), daemon=True).start()
        
    def _download_header(self, appid):
        import urllib.request
        import os
        ...
        try:
            ...
            cover_path = os.path.join(self.proj.get("path", ""), "thub_cover.jpg")
            with open(cover_path, "wb") as f:
                f.write(img_data)
                
            self.after(0, self._finish_and_close)
        except Exception as e:
            self.after(0, lambda: self.lbl_status.configure(text=f"ดาวน์โหลดล้มเหลว: {e}"))
...
class ModderHubApp(ctk.CTk):
    ...
    def force_update_cover(self, proj):
        dialog = ctk.CTkToplevel(self)
        dialog.title(_("การจัดการภาพหน้าปก"))
        ...
        ctk.CTkLabel(dialog, text=_("เลือกวิธีการเปลี่ยนภาพหน้าปก"), font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(20, 15))
        
        def on_steam():
            dialog.destroy()
            # Open the new Preview Dialog!
            SteamSearchPreviewDialog(self, proj, initial_search=proj.get("name", ""))
        ...
        def on_local():
            ...
            import shutil
            import os
            file_path = filedialog.askopenfilename(title=_("เลือกรูปภาพหน้าปก"), filetypes=[("Image Files", "*.jpg *.jpeg *.png")])
            if file_path:
                try:
                    cover_dest = os.path.join(proj.get("path", ""), "thub_cover.jpg")
                    shutil.copy2(file_path, cover_dest)
                    self.show_home()
                except Exception as e:
                    from tkinter import messagebox
                    messagebox.showerror(_("Error"), f"ไม่สามารถก็อปปี้รูปภาพได้: {e}")
        ...
        def on_delete():
            ...
            if os.path.exists(cover_dest):
                if messagebox.askyesno(_("ยืนยัน"), _("คุณต้องการลบภาพหน้าปกของการ์ดนี้ใช่หรือไม่?")):
                    try:
                        os.remove(cover_dest)
                        self.show_home()
                    except Exception as e:
                        messagebox.showerror(_("Error"), f"ไม่สามารถลบภาพได้: {e}")
            else:
                messagebox.showinfo(_("Info"), _("โปรเจกต์นี้ยังไม่มีภาพหน้าปกครับ"))
                
        def on_adjust_crop():
            dialog.destroy()
            AdjustCropDialog(self, proj)
                    
        btn_adjust = ctk.CTkButton(dialog, text=_("📐 จัดตำแหน่งภาพ (Crop)"), fg_color="#f9e2af", text_color="#11111b", hover_color="#fab387", command=on_adjust_crop)
        
        import os
        cover_dest = os.path.join(proj.get("path", ""), "thub_cover.jpg")
        if os.path.exists(cover_dest):
            btn_adjust.pack(fill="x", padx=40, pady=7)
            
        btn_steam = ctk.CTkButton(dialog, text=_("🌐 ค้นหาภาพจาก Steam (แสดงตัวเลือก)"), fg_color="#89b4fa", text_color="#11111b", hover_color="#b4befe", command=on_steam)
        btn_steam.pack(fill="x", padx=40, pady=7)
        
        btn_local = ctk.CTkButton(dialog, text=_("📁 เลือกรูปจากคอมพิวเตอร์"), fg_color="#a6e3a1", text_color="#11111b", hover_color="#94e2d5", command=on_local)
        btn_local.pack(fill="x", padx=40, pady=7)
        
        btn_delete = ctk.CTkButton(dialog, text=_("🗑️ ลบภาพหน้าปกออก"), fg_color="#f38ba8", text_color="#11111b", hover_color="#eba0ac", command=on_delete)
        btn_delete.pack(fill="x", padx=40, pady=7)
        
        btn_cancel = ctk.CTkButton(dialog, text=_("❌ ยกเลิก"), fg_color="transparent", border_width=1, border_color="#cba6f7", text_color="#cba6f7", hover_color="#313244", command=dialog.destroy)
        btn_cancel.pack(fill="x", padx=40, pady=(15, 5))
    def show_home(self):
        ...
        if not projects:
            ...
            for i, proj in enumerate(projects):
                ...
                # Increase card height to 270 so buttons fit
                # Adjust height to 250 so it's not too tall
                # Adjust height to 240 for a tighter look
                card = ctk.CTkFrame(self.projects_frame, width=426, height=280, corner_radius=10, fg_color="#1e1e2e", border_width=1, border_color="#45475a")
                ...
                bottom_frame.pack(fill="x", side="bottom", padx=15, pady=(5, 12))
        
                bottom_frame.grid_columnconfigure((0, 1, 2), weight=1)
                bottom_frame.grid_columnconfigure(3, weight=0)

                # Set width=0 so they expand to fill their grid column
                btn_ai = ctk.CTkButton(bottom_frame, text="🤖 AI Helper", width=0, fg_color="transparent", border_width=1, border_color="#cba6f7", text_color="#cba6f7", hover_color="#313244", command=lambda pj=proj: self.show_ai_dialog(pj))
                ...
                # Add icons if they are default names
                ...
                btn_open = ctk.CTkButton(bottom_frame, text="📁", width=35, fg_color="transparent", border_width=1, border_color="#89b4fa", text_color="#89b4fa", hover_color="#313244", command=lambda p=proj.get("path"): self.open_project_folder(p))
                ...
                if os.path.exists(cover_path):
                    try:
                        ...
                        # Load user's saved offset
                        offset_y = proj.get("cover_offset_y", 0)
                        
                        # Resize to width 426 preserving aspect ratio
                        ratio = 426 / float(img.width)
                        new_height = int(float(img.height) * ratio)
                        img = img.resize((426, new_height), Image.Resampling.LANCZOS)
                        
                        # Calculate crop box (we want a 180px tall view)
                        # Offset determines where the top of the crop starts
                        # Make sure offset doesn't exceed image bounds
                        offset_y = max(0, min(offset_y, new_height - 180))
                        
                        box = (0, offset_y, 426, offset_y + 180)
                        img = img.crop(box)
                        ...
                        draw.text((11, 161), short_p, font=fnt, fill=(17, 17, 27))
                        # Draw text
                        draw.text((10, 160), short_p, font=fnt, fill=(205, 214, 244))
                        
                        ctk_img = ctk.CTkImage(light_image=img, size=(426, 180))
    ...
    def show_card_context_menu(self, event, index, proj):
        ...
        menu.add_command(label=_("🖼️ จัดการรูปหน้าปก").replace('\ufe0f', ''), command=lambda: self.force_update_cover(proj))
...
"""
    ...
    def on_projects_resize(self, event):
        # Use 460 as base, plus we handle left-alignment now so no huge gaps
        usable_width = event.width - 30 
        card_width = 460 
    ...
    def _rearrange_project_cards(self):
        ...
        # Left-align the grid by pushing extra space to a dummy column at the right
        try:
...
"""
    ...
    def check_for_updates_bg(self):
        return # Force disable auto update
